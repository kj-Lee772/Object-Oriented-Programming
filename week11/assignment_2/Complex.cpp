...
Number *Complex::add(Number * num) {
	...
	if (num->types() == INTEGER) {
		Integer* new_number = dynamic_cast<Integer*>(num);
		this->set_val(this->real() + (double)new_number->val(), this->imag());
		delete(new_number);
		return this;
	}
	else if (num->types() == FLOAT) {
		Float* new_number = dynamic_cast<Float*>(num);
		this->set_val(this->real() + new_number->val(), this->imag());
		delete(new_number);
		return this;
	}
	else if (num->types() == COMPLEX) {
		Complex* new_number = dynamic_cast<Complex*>(num);
		this->set_val(this->real() + new_number->real(), this->imag() + new_number->imag());
		delete(new_number);
		return this;
	}
	else {
...
Number *Complex::mul(Number * num) {
	...
	if (num->types() == INTEGER) {
		Integer* new_number = dynamic_cast<Integer*>(num);
		this->set_val(this->real() * (double)new_number->val(), this->imag() * (double)new_number->val());
		delete(new_number);
		return this;
	}
	else if (num->types() == FLOAT) {
		Float* new_number = dynamic_cast<Float*>(num);
		this->set_val(this->real() * new_number->val(), this->imag() * new_number->val());
		delete(new_number);
		return this;
	}
	else if (num->types() == COMPLEX) {
		Complex* new_number = dynamic_cast<Complex*>(num);
		this->set_val(this->real() * new_number->real() - this->imag() * new_number->imag(),
						this->real() * new_number->imag() + this->imag() * new_number->real());
		delete(new_number);
		return this;
	}
	else {

...
Number *Integer::add(Number * num) {
	...
	if (num->types() == INTEGER) {
		...
		this->set_val(this->val() + new_number->val());
		delete(new_number);
		return this;
	}
	else if (num->types() == FLOAT) {
		Float* new_number = dynamic_cast<Float*>(num);
		Float* result = new Float((double)this->val() + new_number->val());
		delete(this);
		delete(new_number);
		return result;
	}
		...
		Complex* result = new Complex((float)this->val()+(float)new_number->real(), (float)new_number->imag());
		delete(this);
		delete(new_number);
		return result;
	}
...
Number *Integer::mul(Number * num) {
	...
	if (num->types() == INTEGER) {
		...
		Complex* new_number = dynamic_cast<Complex*>(num);
		Complex* result = new Complex((float)this->val() * (float)new_number->real(), (float)new_number->imag() * (float)this->val());
		delete(this);
		delete(new_number);
		return result;
	}
